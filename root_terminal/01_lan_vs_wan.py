...
class LanVsWan(Scene):
    def construct(self):
        ...
        wan_link = Line(router1.get_center(), router2.get_center(), color=ELECTRIC_RED)
        wan_link2 = ArcBetweenPoints(router2.get_center(), router3.get_center(), angle=45 * DEGREES, color=ELECTRIC_RED)
        ...
        routing_table = Table(
            table_data,
            col_labels=[Text("LAN", color=MATRIX_GREEN), Text("WAN", color=ELECTRIC_RED)],
